...
import { ConsistencyTestingService, ConsistencyTestResult } from './consistency-testing.service';

@Controller('consistency')
export class ConsistencyTestingController {
  constructor(private readonly consistencyTestingService: ConsistencyTestingService) {}
  ...
  async testMoodConsistency(
    ...
    return await this.consistencyTestingService.testMoodConsistency(body.text, runs);
  }
  ...
  async testEnergyConsistency(
    ...
    const runs = body.runs || 5;
    return await this.consistencyTestingService.testEnergyConsistency(body.text, runs);
  }
  ...
  async testNutritionConsistency(
    ...
    const runs = body.runs || 5;
    return await this.consistencyTestingService.testNutritionConsistency(body.text, runs);
  }
  ...
  async testTriggersConsistency(
    ...
    const runs = body.runs || 5;
    return await this.consistencyTestingService.testTriggersConsistency(body.text, runs);
  }
  ...
  async testPresetEntry(
    ...
    const sampleTexts = {
      'addiction-recovery': 'Day 1. Hands shaking. Made eggs and toast. Dave destroyed my presentation today - felt my chest get tight, wanted a drink so bad. Called Sarah instead.',
      'nutrition': 'Starting this food journal thing my doctor recommended. Breakfast was rushed - just grabbed a banana and coffee on the way to work. Lunch was that quinoa salad from the place downstairs, pretty good actually.',
      'mental-health': 'Therapy was intense today. We dug into some childhood stuff I haven\'t thought about in years. Dr. Martinez says it\'s normal to feel stirred up after sessions like this.',
    };
